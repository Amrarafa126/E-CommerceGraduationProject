...
namespace E_Commerce.Core.Features.AdminDashboard.Queries.Handlers
{
        ...
        : IRequestHandler<GetFinancialSummaryQuery, ApiResponse<FinancialSummaryDto>>
    {
            ...
            GetFinancialSummaryQuery req, CancellationToken ct)
        {
            ...
            var totalGmv = await db.orderSubOrders
                .Where(s => !s.IsDeleted && s.Status != OrderStatus.Cancelled && s.Status != OrderStatus.Refunded)
                .SumAsync(s => (decimal?)s.TotalAmount, ct) ?? 0;

            var refundedAmount = await db.orderSubOrders
                .Where(s => !s.IsDeleted && s.Status == OrderStatus.Refunded)
                .SumAsync(s => (decimal?)s.TotalAmount, ct) ?? 0;
