...
namespace E_Commerce.Core.Features.Chats
{
    ...
    public record MessageDto(Guid Id, Guid ConversationId, Guid SenderId, string SenderName, string Content, string? AttachmentUrl, int Type, bool IsRead, DateTime? ReadAt, DateTime CreatedAt);
    public record SendMessageDto(string Content, string? AttachmentUrl = null, int Type = 0);
