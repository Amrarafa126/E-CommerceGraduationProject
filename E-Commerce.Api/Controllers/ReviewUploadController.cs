    ...
    public class ReviewUploadController(IWebHostEnvironment env) : ControllerBase
    {
        ...
        public async Task<IActionResult> Upload(IFormFile file, CancellationToken ct)
        {
            ...
            if (file.Length > ImageRule.MaxSize)
                return BadRequest(ApiResponse<object>.Fail($"حجم الصورة يتجاوز الحد الأقصى المسموح به {ImageRule.MaxSize / 1024 / 1024} ميجابايت.", 400));

            if (!ImageRule.MimeTypes.Contains(file.ContentType.ToLower()))
                return BadRequest(ApiResponse<object>.Fail($"نوع الصورة غير صالح: {file.ContentType}.", 400));
